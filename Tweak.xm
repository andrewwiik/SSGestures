...
-(CGVector)_translationAmountForState:(NSUInteger)state pileTranslation:(CGPoint)pileTranslation {
	...
	if (fakePileTap & !state) {
		...
		SSSScreenshotsViewController *ssController = ((SSSScreenshotsViewController *)MSHookIvar<UIViewController *>(self, "_screenshotsViewController"));
		...
		CGRect frame7 = [self.view convertRect:ssView.bounds fromView:ssView];
		CGRect frame8 = [self.view convertRect:ssView.frame fromView:[ssView superview]];
		CGRect frame9 = [self.view convertRect:[ssView currentlyVisibleRect] fromView:[ssView superview]];
		CGFloat transX = self.view.center.x - CGRectGetMidX(borderFrame) + 32.0;
		CGFloat transY = self.view.center.y - CGRectGetMidY(borderFrame) - 32.0*2;
		UIView *newView = [[UIView alloc] initWithFrame:CGRectMake((frame7.origin.x + frame8.origin.x)* 0 + frame9.origin.x , 0, borderFrame.size.width,50)];
		newView.backgroundColor = [UIColor greenColor];
		newView.alpha = 0.5;
		CGVector orig = %orig(state, CGPointMake(transX,transY));
		return orig;
    ...
    if (([gestureRecognizer state] != UIGestureRecognizerStateEnded && [gestureRecognizer state] != UIGestureRecognizerStateFailed) && [gestureRecognizer state] != UIGestureRecognizerStatePossible) {
    	...
    	if (self.isVertical) {
    		...
    		if (fabs(translation.x) > fabs([self _pileTranslationAmountForDismiss])) {
    			...
    			UIViewController *screenshotsController = ((UIViewController *)MSHookIvar<UIViewController *>(self, "_screenshotsViewController"));
...
- (BOOL)_pileTranslationIsTowardsOppositeEdge {
	...
	if ([self.view effectiveUserInterfaceLayoutDirection]) return translation.x < 0.0;
